...
function renderDashboardHtml(state) {
    const latestEvent = (0, components_1.findLatestFileEvent)(state.fileState, state.timeline);
    ...
    const body = `
            ...
            <article class="panel strong span-8">
                <div class="panel-header">
                    <div>
                        ...
                        <div class="panel-meta">${(0, html_1.escapeHtml)(state.fileUri || 'No active file selected')} ${state.loading ? '• refreshing…' : ''}</div>
                    </div>
                    <div class="chip-row">
        ...
        confidence: 0,
            ...
            <article class="panel soft span-4">
                ...
                <div class="panel-body">
                    ${(0, components_1.renderTrustNarrative)(result, state.fileState)}
                    ${(0, components_1.renderFixCard)(result)}
                </div>
            </article>
        </section>
        ...
        <section class="grid">
            <article class="panel span-7">
                ...
                <div class="panel-body">
                    ${currentScores ? `<div class="metric-grid">${(0, components_1.renderDimensionCards)(currentScores)}</div>` : '<div class="empty-state">No active file state available yet.</div>'}
                </div>
            </article>
            <article class="panel soft span-5">
                ...
                <div class="panel-body">
                    ${latestEvent ? (0, components_1.renderDiffCard)(latestEvent.diff) : '<div class="empty-state">Run at least two revisions to show score movement and fixed findings.</div>'}
                </div>
            </article>
        </section>
        ...
        <section class="grid">
            <article class="panel span-8">
                ...
                <div class="panel-body">${(0, components_1.renderTimeline)(state.timeline)}</div>
            </article>
            <article class="panel soft span-4">
                ...
                <div class="panel-body">
                    ${dashboard ? `<div class="metric-grid">${(0, components_1.renderDimensionCards)(dashboard.average_scores)}</div>` : '<div class="empty-state">No dashboard data yet. Analyze a supported file to generate the first trust snapshot.</div>'}
                </div>
            </article>
        </section>
        ...
        <section class="two-col">
            <article class="panel span-6">
                ...
                <div class="panel-body">
                    ${(0, components_1.renderAnalysisProfile)(result?.analysis_profile ?? null)}
                </div>
            </article>
            <article class="panel soft span-6">
                ...
                <div class="panel-body">
                    ${state.error ? `<div class="empty-state">${(0, html_1.escapeHtml)(state.error)}</div>` : (0, components_1.renderAgentTrace)(result?.agent_trace ?? [])}
                </div>
            </article>
        </section>`;
    return (0, theme_1.createAppShell)('Savier Trust Cockpit', body);
}
